...
times = [[] for i in range(2020-2006+1)]
minTimes = [[] for i in range(2020-2006+1)]
maxTimes = [[] for i in range(2020-2006+1)]  #list of lists of t_since_t_0

numberOfBursts = [0 for i in range(2020-2006+1)]



#medianTime is a list of lists of gcn times since t_0. index 0 -> 2006; index 14 -> 2020

allTimes = [] #flat list of every t-t_0, when it's convenient to use
for (burstCode, gcnData) in burstDict.items(): #gcnData is a list of lists [[GCN, date time, time since dt_0]]
...
# #plot of all t-t_0 for 2006-2020

# allTimes.sort()
# plt.hist([np.log10(time) for time in allTimes], bins = int(len(allTimes)**0.5), histtype='step', lw=2)
...
# allTimes.reverse()
# plt.plot([np.log10(time) for time in allTimes],[(index+1)/len(allTimes) for index in range(len(allTimes))])
# plt.xlabel('Log($t-t_0$) (s)')
# plt.ylabel('F($t-t_0$)')
...
#histogram showing circulars vs. t-t_0 for sum of all years 2006-2020
